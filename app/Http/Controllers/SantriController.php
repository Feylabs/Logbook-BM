...
class SantriController extends Controller
{
    ...
    function storeSelf(Request $request){
        $rules = [
        ...
        $object->nama = $request->nama;
        $object->jk = $request->gender;
        $object->line_id = $request->line_id;
        $object->no_telp = $request->contact;
        ...
        if($object){
            return redirect('/login')->with(["success" => "Registrasi Berhasil, Silakan Login Untuk Menggunaan Akun"]);
        }else{
            return back()->with(["error" => "Pendaftaran Gagal, Silakan Coba Lagi Nanti"]);
        }
